...
shared_ptr<Mesh> Mesh::CreateMesh()
{
    ...
    //정점타입
    temp->vertexType = VertexType::PC;
    //도형정의
    temp->primitiveTopology = D3D10_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
    //정점당 크기
    temp->byteWidth = sizeof(VertexPC);


    //정점 갯수
    temp->vertexCount = 4 * 6;

    //인덱스 갯수
    temp->indexCount = 6 * 2 * 3;

    //정점 데이터 생성
    VertexPC* Vertex = new VertexPC[temp->vertexCount];
    ...
    for (int i = 0; i < 4; i++)
    {
        Color color = Color(RANDOM->Float(), RANDOM->Float(), RANDOM->Float());
        Color color2 = Color(RANDOM->Float(), RANDOM->Float(), RANDOM->Float());


        Matrix rotY = Matrix::CreateRotationY(PI * 0.5f * i);
        Vertex[i * 4 + 0].position = Vector3::Transform(Vertex[0].position , rotY);
        Vertex[i*4+1].position = Vector3::Transform(Vertex[1].position, rotY);
        Vertex[i*4+2].position = Vector3::Transform(Vertex[2].position, rotY);
        Vertex[i*4+3].position = Vector3::Transform(Vertex[3].position, rotY);
        Vertex[i * 4 + 0].color = color;
        Vertex[i * 4 + 1].color = color;
        Vertex[i * 4 + 2].color = color2;
        Vertex[i * 4 + 3].color = color2;
    }
    for (int i = 0; i < 2; i++)
    {
        Color color = Color(RANDOM->Float(), RANDOM->Float(), RANDOM->Float());
        ...
        Matrix rotX = Matrix::CreateRotationX( (i+0.5f)*PI );
        Vertex[i * 4 + 0+16].position = Vector3::Transform(Vertex[0].position, rotX);
        Vertex[i * 4 + 1+16].position = Vector3::Transform(Vertex[1].position, rotX);
        Vertex[i * 4 + 2+16].position = Vector3::Transform(Vertex[2].position, rotX);
        Vertex[i * 4 + 3+16].position = Vector3::Transform(Vertex[3].position, rotX);
        Vertex[i * 4 + 0+16].color = color;
        Vertex[i * 4 + 1+16].color = color;
        Vertex[i * 4 + 2+16].color = color2;
        Vertex[i * 4 + 3+16].color = color2;
    }
    ...
    for (int i = 0; i < 6; i++)
    {
        temp->indices[i*6 +0] = i*4 +0;
        temp->indices[i*6 +1] = i*4 +1;
        temp->indices[i*6 +2] = i*4 +3;

        temp->indices[i * 6+3] = i * 4 +1;
        temp->indices[i * 6 + 4] = i * 4+ 2;
        temp->indices[i * 6 + 5] = i * 4+ 3;
    }
